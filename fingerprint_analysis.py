...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  1 14:02:22 2022
...
def smoothing(img):
    ...
    """
    final = closing(opening(img)).astype(bool)

    return final

def skeletonise(img):
    img_inv = np.invert(img)
    img_skeleton = skimage.morphology.skeletonize(img_inv)
    img_skeleton = np.invert(img_skeleton)
    
    return img_skeleton


img1 = plt.imread('set1_2.tif')
...
img1_smoothed_cleaned = morph.remove_small_holes(img1_smoothed, 
                                                 area_threshold=15, 
                                                 connectivity=5)

img1_skeletonised = skeletonise(img1_smoothed_cleaned) # Uses Zhang method
...
ax_smoothed = fig.add_subplot(337)
...
ax_smoothed.imshow(img1_smoothed_cleaned, cmap='gray')
